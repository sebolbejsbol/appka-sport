-- Migracja 0005: pełniejszy filtr koszykówki (boiska z koszem, nie tylko sport=basketball)
-- Uruchom w Supabase: SQL Editor → New query → wklej całość → Run.
--
-- Wcześniej mapa pokazywała tylko dokładnie sport='basketball' (~250 punktów).
-- W bazie jest już ~398 boisk z koszem w Trójmieście, m.in.:
--   - sport=basketball;soccer (boiska wielofunkcyjne)
--   - sport=multi (Orliki / MUGA — prawie zawsze mają kosz)
--   - place zabaw z koszem (leisure=playground)
