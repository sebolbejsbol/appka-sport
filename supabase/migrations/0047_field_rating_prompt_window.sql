-- ═══════════════════════════════════════════════════════════════════════════
-- 0047 — Okno oceny boiska tylko tuż po evencie (90 min)
-- ═══════════════════════════════════════════════════════════════════════════

drop function if exists public.my_events_pending_field_rating(integer);
...
as $$
  ...
  where public.user_can_rate_event_field(e.id)
    ...
    and now() <= e.starts_at + (e.duration_min || ' minutes')::interval + interval '90 minutes'
