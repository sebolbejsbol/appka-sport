-- ═══════════════════════════════════════════════════════════════════════════
-- 0014 — Fix: widoczność listy eventów
-- Kategoria: Eventy  |  Typ: FIX  |  Wymaga: 0012
-- Plik: supabase/migrations/0014_events_list_fix.sql
-- SQL Editor: 0014 — fix listy eventów
-- ═══════════════════════════════════════════════════════════════════════════
-- Uruchamiaj w kolejności numerów. Skrypt idempotentny (bezpieczny ponownie).
-- UWAGA: migracja naprawcza — uruchom po 0012.
-- Zastępuje w SQL Editor: "naprawka do migracji 8 i 13"
...
as $$
  ...
  where e.status = 'planned'
    -- Jak events_for_field: nadchodzące + trwające (do 3 h po starcie)
