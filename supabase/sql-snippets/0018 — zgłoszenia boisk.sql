-- ═══════════════════════════════════════════════════════════════════════════
-- 0018 — Zgłoszenia boisk przez użytkowników
-- Kategoria: Boiska  |  Typ: BASE  |  Wymaga: 0002, 0013
-- Plik: supabase/migrations/0018_user_field_reports.sql
-- SQL Editor: 0018 — zgłoszenia boisk
-- ═══════════════════════════════════════════════════════════════════════════
...
comment on column public.fields.submitted_by is 'Kto zgłosił boisko (gdy source = user).';
comment on column public.fields.user_note is 'Uwagi od zgłaszającego (widoczne dla admina).';

-- Zgłoszenie nowego boiska (tylko zalogowani, przez RPC — omija brak polityki INSERT)
...
begin
  ...
  -- Nie dubluj zgłoszeń / boisk w promieniu 40 m
  if exists (
    select 1
  ...
  -- Max 10 oczekujących zgłoszeń na użytkownika
  if (
    select count(*)
    from public.fields f
...
grant execute on function public.submit_field_report(
  double precision, double precision, text, text, text
) to authenticated;

-- Kolejka admina: uwagi zgłaszającego + nick
-- CREATE OR REPLACE nie zmienia typu zwracanego — najpierw usuń starą wersję z 0013.
